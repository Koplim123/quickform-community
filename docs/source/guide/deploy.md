...
为了让更多教师最快速度用上QuickForm，体验AI赋能教学带来的各种便利，我们购买了云服务器，申请了域名，部署了线上的版本（演示地址：[https://quickform.cn](https://quickform.cn)），注册后就能体会QuickForm的强大功能。
...
本地部署看起来麻烦，但优势很明显。我们还为其设计了一些特色功能。

### 3.1 本地部署的优势与不足

本地部署QuickForm拥有很多优势，如：

1）没有流量限制。QuickForm在线版出于无奈，只能用各种方法限制网络流量，不然再好的服务器也无法支撑众多老师的频繁使用。

2）数据相对安全。本地部署后，数据都存在本地了，肯定要安全多了。为什么不说绝对安全？只要联网就有安全隐患。

本地部署的QuickForm也存在弊端，那就是制作数据大屏、调试交互功能时，容易出现种种问题。因为大模型不知道通过API地址读出的数据是怎样的格式，毕竟大模型没法访问内网地址。因此，扣子编程上的“QF数据大屏”技能也不能使用了。

### 3.2 QF数据互联功能

为了解决网页调试难的问题，我们提供了“QF数据互联”功能。用户在本地部署的QuickForm中，输入QuickForm在线版的用户名和密码，就能将云端的数据任务导入到本地。在导入数据任务的同时，会将相关的附件也下载过来，甚至贴心地将API地址全部更改为本地版本的地址。也就是说，导入后直接可以使用！

![](../images/guide/deploy02-1.png)

![](../images/guide/deploy02-2.png)

我们推荐的最佳路线：“在线版”新建任务 -> “在线版”调试网页 -> “在线版”上传网页 -> “本地版”导入任务

### 3.3 支持本地大模型

既然在本地部署了，那就“本地”到底吧。本地部署的QuickForm“AI配置”支持Ollama本地AI模型连接，能省钱就省钱吧。只要你的电脑性能还可以，那就试试本地部署大模型。
