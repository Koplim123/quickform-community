...
class Task(Base):
    ...
    file_name_2 = Column(String(200))
    file_path_2 = Column(String(500))
...
class AIConfig(Base):
    ...
    siliconflow_api_key = Column(String(200))
...
def call_ai_model(prompt, ai_config):
    ...
    if ai_config.selected_model == 'deepseek':
        ...
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {ai_config.glm_api_key}"
        }
        data = {
            "model": "glm-4",
            "messages": [
        ...
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {ai_config.siliconflow_api_key}"
        }
        data = {
...
def edit_task(task_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            remove_file_2 = request.form.get('remove_file_2')
            ...
            if 'file_2' in request.files and request.files['file_2'].filename != '':
                ...
                unique_filename_2, filepath_2 = save_uploaded_file(file_2)
                if unique_filename_2:
                    # 删除旧文件
                    if task.file_path_2 and os.path.exists(task.file_path_2):
                        os.remove(task.file_path_2)
                    task.file_name_2 = file_2.filename
                    task.file_path_2 = filepath_2
            # 处理文件2删除
            elif remove_file_2:
                if task.file_path_2 and os.path.exists(task.file_path_2):
                    os.remove(task.file_path_2)
                task.file_name_2 = None
                task.file_path_2 = None
        ...
        if request.method == 'POST':
            ...
            if 'selected_model' in request.form:
                ...
                if ai_config:
                    ...
                    ai_config.siliconflow_api_key = siliconflow_api_key
                else:
                    ai_config = AIConfig(
...
def smart_analyze(task_id):
    ...
    try:
        ...
        if ai_config and ai_config.selected_model:
            if ai_config.selected_model == 'deepseek' and ai_config.deepseek_api_key:
                has_api_key = True
            elif ai_config.selected_model == 'doubao' and ai_config.doubao_api_key:
                has_api_key = True
            elif ai_config.selected_model == 'qwen' and ai_config.qwen_api_key:
                has_api_key = True
            elif ai_config.selected_model == 'glm' and ai_config.glm_api_key:
                has_api_key = True
            elif ai_config.selected_model == 'siliconflow' and ai_config.siliconflow_api_key:
                has_api_key = True
